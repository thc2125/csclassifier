class CorpusCSLangsTestCase(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.test_label2idx = {'lang1':0, 'lang2':1, 'punct':2, 'other':3}
        self.test_idx2label = {i:l for l, i in self.test_label2idx.items()}

        self.test_cs_label2idx = {'<PAD>':0, 'no_cs':1, 'cs':2}
        self.test_cs_idx2label = {i:l for l, i in self.test_label2idx.items()}
        ...
        self.np_labels1 = np.array([
                ([1] * 6) + [4], + ([1]*2) + ([4] * 4) + [2] + [4],
                [1, 4] + ([1] * 2) + [4, 1] + ([4] * 4) + ([0] * 5),
                [1, 4, 1, 1, 4, 1, 1, 4, 1, 4, 1, 4, 4, 4] + [0],
                [1, 1, 4, 4, 4] + ([0] * 10),
                [1, 1, 1, 1, 4, 1, 4, 2] + ([0] * 7),
                [1, 2, 1, 2] + ([0] * 11)])
        self.maxsentlen1 = 15
        ...
        self.np_labels2 = np.array([
            [3, 0,, 0, 3, 0, 1, 0, 0, 0, 3, 0, 3, 3, 0, 0, 0, 0, 3, 0],
    ...
    def test_read_corpus_cs_labels1(self):
        self.assertEqual(sorted(self.corpus1.cs_labels), sorted(self.cs_labels1))

    def test_read_corpus_cs_labels2(self):
        self.assertEqual(sorted(self.corpus2.cs_labels), sorted(self.cs_labels2))
